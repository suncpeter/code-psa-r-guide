...
* List of R software for propensity score analysis: https://www.biostat.jhsph.edu/~estuart/propensityscoresoftware.html
...
* Mahalanobis metric matching can be used to find out matched pairs where the number of units is small
...
# Estimate ATT with marginaleffects::avg_comparisons()
...
Alternatively, balance can be assessed using the `cobalt` package. In addition to mean differences, variance ratios can be requested. Common thresholds for balanced groups are .5 and 2, but it will be close to 1 when group variances are similar.
...
psw_df2 <- psw_df %>% mutate(weights = psw_g1$weights)
...
psw_g2 <- lm(lwss97 ~ kuse + male + black + age97 + pcged97 + mratio96,
  data = psw_df2, weights = weights
)
marginaleffects::avg_comparisons(
  psw_g2,
  variables = "kuse",
  vcov = ~pcg_id,
  newdata = subset(psw_df2, kuse == 1),
  wts = "weights"
)
```
