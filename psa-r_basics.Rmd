...
**Causal inference** is inference about *counterfactual outcomes* or *potential outcomes*.

(*Notation:* Let $w$ represent the dichotomous treatment variable, where 1 = treated and 0 = untreated, let $Y$ represent a dichotomous outcome variable. Thus, $Y_{1i}$ is the potential outcome for unit $i$ with treatment, and $Y_{0i}$ is the potential outcome for unit $i$ without treatment.)
...
pacman::p_load(
  haven, sjlabelled, cobalt, MatchIt, tidyverse, kableExtra, survival, psych
)
```
...
# Load Data
gm_df0 <- haven::read_dta("data/chpt5_1_original.dta")
...
Greedy nearest neighbor matching forms matches by pairing a control participant with a treated participant, if the absolute difference of their propensity scores is the smallest among all possible pairs of propensity scores.
...
```{r message=F, warning=F, error=F}
pacman::p_load(optmatch, knitr, broom, sandwich, rlang)
select <- dplyr::select
```
