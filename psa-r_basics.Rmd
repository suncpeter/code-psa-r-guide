...
# visual
d <- d %>%
  dplyr::mutate(treatfac = case_when(
    kuse == 0 ~ "Nonusers",
    kuse == 1 ~ "Users"
  ))
...
```{r message=F, warning=F, error=F, fig.width=4, fig.height=3, fig.align="center"}
cds <- cds %>%
  dplyr::mutate(treatfac = case_when(
    kuse == 0 ~ "Nonusers",
    kuse == 1 ~ "Users"
...
ggplot(cds, aes(x = treatfac, y = ps)) +
  geom_boxplot() +
  labs(title = "Box Plots of Estimated Propensity Scores")
...
# variable match, (vm3 - use Hensen's equation)
...
```{r}
df.h <- haven::read_dta("data/opt/chpt5_2(original).dta") %>%
...
```{r}
df.h2 <- haven::read_dta("data/opt/chpt5_2(original).dta")
...
For an easier interface for the `optmatch` package, the `MatchIt::matchit()` function also implements optimal full matching and optimal pair matching with the `method = "full"` and `method = "optimal"` arguments, respectively. Variable ratio matching can be set with the `ratio` argument. However, for finer control of the matching process, use the functions `optmatch::fullmatch()` and `optmatch::pairmatch()`.

Below is an illustration of optimal pair matching using generalized boosted model to generate the propensity scores:
